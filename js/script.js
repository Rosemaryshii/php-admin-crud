  ...
  find_prod_ajax = $.ajax({
    ...
    success: function (resp) {
      ...
      Object.keys(resp).map((k) => {
        ...
        item
          .find(".prod_price")
          .text(parseFloat(resp[k].price).toLocaleString("en-US"));
...
function item_actions(_this, key) {
  _this.find(".rem-item").click(function () {
    if (!!listed[key]) {
      _this.remove();
      delete listed[key];
      listed = Object.keys(listed).map((k) => {
        return listed[k] != null ? listed[k] : false;
      });
...
function load_list() {
  listed = !!localStorage.getItem("listed")
    ? $.parseJSON(localStorage.getItem("listed"))
    : [];
  total = 0;
  ...
  if (Object.keys(listed).length > 0) {
    Object.keys(listed).map((k) => {
      ...
      item
        .find(".item-price")
        .text(parseFloat(listed[k].price).toLocaleString("en-US"));
      item
        .find(".item-total")
        .text(
            Math.trunc(
      ...
      total += Math.trunc(
        parseFloat(listed[k].price) *
          ...
          parseFloat(listed[k].width) <= 6.9
            ...
            ? 9
            : parseFloat(listed[k].width) >= 109 &&
              parseFloat(listed[k].width1) <= 120
            ? 10
            : parseFloat(listed[k].width) >= 121 &&
              parseFloat(listed[k].width) <= 133
            ? 11
    ...
    if (change >= 0) {
      $.ajax({
        ...
        success: function (resp) {
          ...
          Object.keys(listed).map((k) => {
            if (!appendedValues) {
              el.find("#cname-value").append(
                '<div class="col-4 text-start"><label for="cname">Name:</label> ' +
                  cnameValue +
                  "</div>"
              );
              el.find("#cname-value").append(
                '<div class="col-4 text-start"><label for="cinvoice">Invoice:</label> ' +
                  cinvoiceValue +
                  "</div>"
              );

              el.find("#cname-value").append(
                '<div class="col-4 text-start"><label for="cname">Phone No:</label> ' +
                  cphoneValue +
                  "</div>"
              );
              appendedValues = true;
            ...
            el.find("#product-list").append(
              '<div class="col-2 text-start lh-1">' +
                listed[k].product +
                "</small></div></div>"
            );
            el.find("#product-list").append(
              '<div class="col-1 text-start">' +
                parseFloat(listed[k].length1).toLocaleString("en-US") +
                "</div>"
            );
            el.find("#product-list").append(
              '<div class="col-1 text-start">' +
                parseFloat(listed[k].width).toLocaleString("en-US") +
                "</div>"
            );
            el.find("#product-list").append(
              '<div class="col-1 text-start">' +
